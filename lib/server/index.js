...
app.post("/register", (req, res) => {
  ...
  db.query(sql, [username, password, usn, firstName, lastName, email], (err, result) => {
    ...
    res.status(200).json({
      message: "Registration successful! Please wait approximately 2 hours for admin approval. You will be able to login once your account is approved.",
      status: "pending",
      waitTime: "2 hours"
    });
  });
...
app.post('/borrow', (req, res) => {
  ...
  // First check if this user already has an active borrow for this book
