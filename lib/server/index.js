...
app.get('/borrowed-books', (req, res) => {
  ...
  const sql = `
    SELECT b.*, bb.borrow_date, bb.expiry_date, b.status
    FROM borrowed_books bb
    JOIN books b ON bb.book_id = b.id
    WHERE bb.user_id = ? AND bb.returned_at IS NULL AND CURRENT_DATE() <= bb.expiry_date
  `;
...
app.post('/sell-book', (req, res) => {
  ...
  const sql = `
    ...
    (user_id, type, title, author, description, acc_no, contact, status)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `;
...
app.use('/api', sellBooksRoutes);


// Mount the router
app.use('/', sellBooksRoutes);
