...
app.post("/register", (req, res) => {
  ...
  if (!username || !password || !usn) {
    return res.status(400).json({ message: "Username, password, and USN are required" });
  }

  // Set default values for optional fields
  const finalFirstName = firstName || 'User';
  const finalLastName = lastName || '';
  const finalEmail = email || `${username}@temp.com`;
  ...
  db.query(sql, [username, password, usn, finalFirstName, finalLastName, finalEmail], (err, result) => {
    if (err) {
      console.error("Error inserting user:", err);
