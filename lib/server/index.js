...
app.use('/api/sell', sellBooksRoutes);      // Book selling routes
app.use('/api/admin', adminRoutes);         // Admin login, borrowed books, add book, etc.

// --- User Authentication Routes ---
app.post("/register", (req, res) => {
  const { username, password, usn } = req.body;
  ...
  const sql = `
    INSERT INTO users (username, password, usn)
    VALUES (?, ?, ?)
  `;

  db.query(sql, [username, password, usn], (err, result) => {
    if (err) {
      console.error("Error inserting user:", err);
      return res.status(500).json({ message: "Registration failed" });
    }
  ...
  if (req.session.user) {
    ...
    db.query(sql, [req.session.user.id], (err, result) => {
      ...
      const profileData = {
        ...
        username: user.username,
        usn: user.usn,
        profileImage: user.profile_image ? `http://localhost:5000${user.profile_image}` : null
      };
